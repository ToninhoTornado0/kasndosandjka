...
//declaranddo variaveis para armazenar as posicoes no botão 
var topMod = 0
var leftMod = 0
var mobileState = 0

//funcao que indentifica qual dispositivo esta sendo renderizado
...
//definindo estados da posição do botao no mobile
...
//VERIFICADOR PARA DIRECIONAR AS FUNÇÕES
...
function mobileHandler (event){
    ...
    moblestate = mobileState % 4 + 1 //mudando o estado
    //mudando o css
    $("#not").css({
...
//definindo uma area de respiro no nosso botao
button.right = button.left  + 100
button;bottom = button.top + 28
button.centerX = button.left + 50
button.centeY = button.top + 14

let distance = Math.sqrt(Math.pow(event.pageX - button.centerX,2)
    + Math.pow9event.pageY - button.centerY,2

)

//condicoes para caso o curso esteja a menos de 80px do botao
...
if (evente.pageX < button.centerX) {
    leftMod -= 10 * model.cos

}else {
    leftMode -= 10* model.cos
}

//modificando o a posição pelo css do botao not 
$("#not").css({top: topMod,left: leftMod,position:'relative'})
}

//funcao que vai calcular o angulo do seno e do cosseno(sin e cos)
